...
client.on("guildMemberRoleAdd", (member, role) => {
    ...
    if(!main) return console.log("Sunucuya Sokmayı Unuttun Botu!")
    let sorumluluk = client.guilds.cache.get(ids.sorumluluk)
    if(!sorumluluk) return console.log("Sorumluluk Sunucusuna Sok!")
    let yetkili = client.guilds.cache.get(ids.yetkili)
    if(!yetkili) return console.log("Yetkili Sunucusuna Sok!")
...
client.on('ready', async () => {
    ...
    client.user.setActivity('Creatéd by hamdibicr', { type: 'PLAYING' });
})
